...
class HashTable {
  ...
  bool find(V item) {
    ...
    for (int i = 0; i <= item.length()-1; i++) {
      int tempNum = item.length()-1-i;
      hashVal = hashVal + ((int)(item.at(i))) * (pow(31,tempNum));
    }

    tempVal = abs((hashVal) % (10007));
    SLL <string> * temp = &table[tempVal];
  ...
  bool insert(V item1, V item2) {
    ...
    for (int i = 0; i <= item1.length()-1; i++) {
      int tempNum = item1.length()-1-i;
      hashVal = hashVal + ((int)(item1.at(i))) * (pow(31,tempNum));
    }

    tempVal = abs((hashVal) % (10007));
    SLL <string> * temp1 = &table[tempVal];

    if(temp1 -> search (item1) != NULL) {
      return false;
    }

    SLL <string> * temp2 = &table[tempVal];
    temp2 -> insert(item1,item2);
    SLL <string> * temp3 = &table[tempVal];
